...
//go:generate bash -c "cd ../../forest-sfc && git checkout c1d33c81f74abf82c0e22807f16e609578e10ad8"
//go:generate bash -c "docker run --name go-forest-sfc-compiler -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../forest-sfc:/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4' && docker commit go-forest-sfc-compiler go-forest-sfc-compiler-image && docker rm go-forest-sfc-compiler"
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../forest-sfc:/src -w /src go-forest-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; rm -f /src/build/contracts/*json; npm run build'"
...
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../forest-sfc:/src -w /src go-forest-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
//go:generate bash -c "cd ../../forest-sfc && git checkout -- truffle-config.js; docker rmi go-forest-sfc-compiler-image"
...
import (
	...
	"github.com/greenchainearth/go-forest/forest/genesis/driver"
	"github.com/greenchainearth/go-forest/forest/genesis/driverauth"
	"github.com/greenchainearth/go-forest/forest/genesis/evmwriter"
	"github.com/greenchainearth/go-forest/forest/genesis/netinit"
	"github.com/greenchainearth/go-forest/forest/genesis/sfc"
	"github.com/greenchainearth/go-forest/gossip/contract/driver100"
	"github.com/greenchainearth/go-forest/gossip/contract/driverauth100"
	"github.com/greenchainearth/go-forest/gossip/contract/netinit100"
	"github.com/greenchainearth/go-forest/gossip/contract/sfc100"
	"github.com/greenchainearth/go-forest/logger"
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("SFC upgrade", func(t *testing.T) {
			...
			rr := env.ApplyBlock(nextEpoch,
				env.Contract(admin, utils.ToTree(0), sfc100.ContractBin),
			)
	...
	t.Run("Direct driver", func(t *testing.T) {
		...
		rr := env.ApplyBlock(nextEpoch,
			env.Contract(admin, utils.ToTree(0), anyContractBin),
		)
		require.Equal(1, rr.Len())
		require.Equal(types.ReceiptStatusSuccessful, rr[0].Status)
		newImpl := rr[0].ContractAddress
...
func cicleTransfers(t *testing.T, env *testEnv, count uint64) {
	...
	for i := uint64(0); i < count; i++ {
		...
		for i := range txs {
			from := (i)%accounts + 1
			to := (i+1)%accounts + 1
			txs[i] = env.Transfer(from, to, utils.ToTree(100))
		}
