...
import (
	...
	"github.com/greenchainearth/go-forest/gossip/evmstore"
	"github.com/greenchainearth/go-forest/gossip/sfcapi"
...
func (s *Service) GetConsensusCallbacks() seed.ConsensusCallbacks {
	return seed.ConsensusCallbacks{
...
func consensusCallbackBeginBlockFn(
	...
	return func(cBlock *seed.Block) seed.BlockCallbacks {
		wg.Wait()
		start := time.Now()
...
func spillBlockEvents(store *Store, block *inter.Block, network forest.Rules) (*inter.Block, inter.EventPayloads) {
...
func mergeCheaters(a, b seed.Cheaters) seed.Cheaters {
	if len(b) == 0 {
		return a
	}
	if len(a) == 0 {
	...
	aSet := a.Set()
	merged := make(seed.Cheaters, 0, len(b)+len(a))
	for _, v := range a {
