...
Contains the scripts to do forest benchmarking (only for fakenet).
...
* Get node1 address:
```sh
go run ../cmd/forest attach --exec "tree.accounts[0]" http://localhost:4001
```
...
```js
tree.sendTransaction(
	{from: tree.accounts[0], to: "0x02aff1d0a9ed566e644f06fcfe7efe00a3261d03", value:  "1000000000"},
...
```sh
tree.getTransactionReceipt("0x68a7c1daeee7e7ab5aedf0d0dba337dbf79ce0988387cf6d63ea73b98193adfd").blockNumber
```
...
```sh
go run ../cmd/forest attach --exec "tree.getBalance(tree.accounts[0])" http://localhost:4000
go run ../cmd/forest attach --exec "tree.getBalance(tree.accounts[0])" http://localhost:4001
```
 outputs:
```js
