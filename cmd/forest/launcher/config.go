...
import (
	...
	"github.com/greenchainearth/go-forest/evmcore"
	"github.com/greenchainearth/go-forest/forest"
	"github.com/greenchainearth/go-forest/forest/genesisstore"
	"github.com/greenchainearth/go-forest/gossip"
	"github.com/greenchainearth/go-forest/gossip/gasprice"
	"github.com/greenchainearth/go-forest/integration"
	"github.com/greenchainearth/go-forest/integration/makegenesis"
	futils "github.com/greenchainearth/go-forest/utils"
...
var (
	...
	// DataDirFlag defines directory to store Seed state and user's wallets
	...
	RPCGlobalGasCapFlag = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in tree_call/estimateGas (0=infinite)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCGasCap,
	}
	RPCGlobalTxFeeCapFlag = cli.Float64Flag{
		Name:  "rpc.txfeecap",
		Usage: "Sets a cap on transaction fee (in TREE) that can be sent via the RPC APIs (0 = no cap)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCTxFeeCap,
	}

	AllowedForestGenesisHashes = map[uint64]hash.Hash{
		forest.MainNetworkID: hash.HexToHash("0x8895b98d25c653773a31be420a6a29d322a10107e76dff37dc694ad02ebacd01"),
		forest.TestNetworkID: hash.HexToHash("0xc4a5fc96e575a16a9a0c7349d44dc4d0f602a54e0a8543360c2fee4c3937b49e"),
	}
)
...
type config struct {
	Node        node.Config
	Forest      gossip.Config
	ForestStore gossip.StoreConfig
	Seed        abft.Config
	SeedStore   abft.StoreConfig
...
func (c *config) AppConfigs() integration.Configs {
	return integration.Configs{
		...
		Seed:           c.Seed,
		SeedStore:      c.SeedStore,
		VectorClock:    c.VectorClock,
		AllowedGenesis: AllowedForestGenesisHashes,
	}
}
...
func getForestGenesis(ctx *cli.Context) integration.InputGenesis {
	...
	case ctx.GlobalIsSet(FakeNetFlag.Name):
		...
		fakeGenesisStore := makegenesis.FakeGenesisStore(num, futils.ToTree(1000000000), futils.ToTree(5000000))
...
func mayMakeAllConfigs(ctx *cli.Context) (*config, error) {
	...
	cfg := config{
		...
		Forest:      gossip.DefaultConfig(cacheRatio),
		ForestStore: gossip.DefaultStoreConfig(cacheRatio),
		Seed:        abft.DefaultConfig(),
		SeedStore:   abft.DefaultStoreConfig(cacheRatio),
		VectorClock: vecmt.DefaultConfig(cacheRatio),
	}

	if ctx.GlobalIsSet(FakeNetFlag.Name) {
		_, num, _ := parseFakeGen(ctx.GlobalString(FakeNetFlag.Name))
		cfg.Forest = gossip.FakeConfig(num, cacheRatio)
	}
	...
	var err error
	cfg.Forest, err = gossipConfigWithFlags(ctx, cfg.Forest)
	if err != nil {
		return nil, err
	}
	cfg.ForestStore, err = gossipStoreConfigWithFlags(ctx, cfg.ForestStore)
	if err != nil {
		return nil, err
	}
	cfg.Node = nodeConfigWithFlags(ctx, cfg.Node)
	if cfg.Forest.Emitter.Validator.ID != 0 && len(cfg.Forest.Emitter.PrevEmittedEventFile.Path) == 0 {
		cfg.Forest.Emitter.PrevEmittedEventFile.Path = cfg.Node.ResolvePath(path.Join("emitter", fmt.Sprintf("last-%d", cfg.Forest.Emitter.Validator.ID)))
	}

	if err := cfg.Forest.Validate(); err != nil {
		return nil, err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "eth", "tree", "dag", "sfc", "abft", "web3")
	cfg.WSModules = append(cfg.WSModules, "eth", "tree", "dag", "sfc", "abft", "web3")
	cfg.IPCPath = "forest.ipc"
	cfg.DataDir = DefaultDataDir()
	return cfg
}
