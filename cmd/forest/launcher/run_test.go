...
import (
	...
	"github.com/greenchainearth/go-forest/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "forest-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "forest-test" in exec().
	reexec.Register("forest-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "forest". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("forest-test", args...)
