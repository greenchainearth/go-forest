...
import (
	...
	"github.com/greenchainearth/go-forest/cmd/forest/launcher/metrics"
	"github.com/greenchainearth/go-forest/cmd/forest/launcher/tracing"
	"github.com/greenchainearth/go-forest/debug"
	"github.com/greenchainearth/go-forest/flags"
	"github.com/greenchainearth/go-forest/gossip"
	"github.com/greenchainearth/go-forest/integration"
	"github.com/greenchainearth/go-forest/utils/errlock"
	"github.com/greenchainearth/go-forest/valkeystore"
	_ "github.com/greenchainearth/go-forest/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "go-forest"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-forest command line interface")
...
func initFlags() {
	...
	nodeFlags = append(nodeFlags, forestFlags...)
...
// forest is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func seedMain(ctx *cli.Context) error {
	...
	genesisPath := getForestGenesis(ctx)
...
func makeNode(ctx *cli.Context, cfg *config, genesis integration.InputGenesis) (*node.Node, *gossip.Service, func()) {
	...
	valPubkey := cfg.Forest.Emitter.Validator.PubKey
	if key := getFakeValidatorKey(ctx); key != nil && cfg.Forest.Emitter.Validator.ID != 0 {
	...
	svc, err := gossip.NewService(stack, cfg.Forest, gdb, signer, blockProc, engine, dagIndex)
	if err != nil {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local forest node.
	...
	if ctx.GlobalBool(utils.ExitWhenSyncedFlag.Name) {
		go func() {
			for first := true; ; first = false {
				// Call tree_syncing until it returns false
				time.Sleep(5 * time.Second)

				var syncing bool
				err := rpcClient.CallContext(context.TODO(), &syncing, "tree_syncing")
				if err != nil {
					continue
				}
				if !syncing {
