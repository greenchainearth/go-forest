...
func DoEstimateGas(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, gasCap uint64) (hexutil.Uint64, error) {
	...
	executable := func(gas uint64) (bool, *evmcore.ExecutionResult, error) {
		...
		result, err := DoCall(ctx, b, args, blockNrOrHash, nil, forest.DefaultVMConfig, 0, gasCap)
		if err != nil {
...
func (api *PrivateDebugAPI) SetHead(number hexutil.Uint64) error {
	return errors.New("seed cannot rewind blocks due to the BFT algorithm")
}
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f TREE) exceeds the configured cap (%.2f TREE)", feeFloat, cap)
	}
	return nil
}
