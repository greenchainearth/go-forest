...
func ApplyGenesis(statedb *state.StateDB, g forest.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g forest.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g forest.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
