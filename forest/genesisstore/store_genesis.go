...
func (s *Store) GetRules() forest.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &forest.Rules{}).(*forest.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg forest.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() forest.Genesis {
	meatadata := s.GetMetadata()
	return forest.Genesis{
